...
At jump discontinuities, the partial sums of a Fourier series exhibit a rather curious misbehavior known as the Gibbs phenomenon: the overshoots near the discontinuity don't vanish as $N \to \infty$, but instead converge to approximately $9\%$ of the jump height. More precisely, if $f$ has a jump of size $J$ at $x_0$, then:
\begin{equation}
...
And there it is---exactly the complex Fourier series! The Laurent coefficients \eqref{eq:laurent_coeff}, evaluated on the unit circle ($\gamma: \zeta = e^{i\phi}$, $\deriv{\zeta} = ie^{i\phi}\deriv{\phi}$), become:
\begin{align}
...
\end{align}
computes two outputs from each pair. Applying this recursively yields $\log_2 N$ stages of $N/2$ butterflies each, for a total of $O(N \log N)$ operations. This is the Cooley--Tukey radix-2 decimation-in-time (DIT) algorithm---elegant, but limited to power-of-two lengths. We'll now generalize.
...
An important structural note: Bluestein's algorithm calls the staged FFT on power-of-two lengths (always composite). The staged FFT dispatches to the generic quadratic butterfly for prime factors \textit{within} a composite $N$---it never calls Bluestein. There is no circular dependency.
...
The discrete analogue holds for the DFT: circular convolution in the time domain corresponds to pointwise multiplication of DFTs. This fact underpins FFT-based filtering, correlation, and polynomial multiplication---essentially any setting where one needs to convolve efficiently.
...
\begin{itemize}
    ...
    \item \textbf{Data compression}: retaining only the largest coefficients---a form of lossy compression, but an effective one.
