...
This perspective also clarifies \textit{why} the Fourier series works: the completeness of the Fourier system $\{e^{in\theta}\}$ follows from the completeness of the Laurent series on the annulus. The linear algebra viewpoint of chapter II and the complex analysis viewpoint heretofore are thus two facets of the same underlying structure---two lenses, as promised, trained on a single object.
...
\section{Bluestein's Algorithm and Dispatch}
\label{sec:bluestein_dispatch}

\subsection{Bluestein's Chirp-Z Transform}
...
\subsection{1-D FFT Dispatch}

The complete 1-D dispatch logic is straightforward:
\begin{equation}
...
The companion implementation of the algorithms described in this section can be found in the author's \texttt{mdarray} library.
...
As $N$ increases, the reconstruction converges to the original contour---a direct consequence of the completeness results from Chapter II. In practice, $N \approx 100\text{--}500$ harmonics suffices for visually faithful reproductions of most images, which is rather remarkable.
...
\begin{figure}[htbp]
    ...
    \caption{Epicycle reconstruction with increasing numbers of harmonics. As $N$ grows, the reconstruction converges to the original shape.}
...
\begin{figure}[htbp]
    ...
    \caption{The contour tracing pipeline: (a) target shape, (b) ordered contour points, (c) contour path, (d) Fourier reconstruction from the extracted contour.}
